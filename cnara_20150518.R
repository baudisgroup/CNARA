...
setMethod("plotFCF", "SpeakCNAno", 
	function(object) {
		...
		axis(1, at=labelTextPos, labels=seq(1:22), tick=FALSE, cex.axis=1)
...
workPath <- setwd("/Users/niai/Dropbox (baudisgroup)/baudisgroup/CNAQA")
...
trainingFile <- file.path(workPath, "cnaqa.training.tsv")
outputFile <- file.path(workPath, "cnaqa.out2.tsv")

arrayDir <- file.path(samplePath, series)
arrayList <- list.files(arrayDir)
noArray <- length(arrayList)

tableHeader <- paste("Sample", "Series", "Platform", "Speak", "Breakpoint_CNA", "Breakpoint_CBS", "Spread", "Classification_Label", "Decision_Value", "Good_Poor", "Case_Diagnosis", sep = "\t")
...
for (arr in 1:noArray) {
	...
	tmp <- paste(arrayList[arr], series, platform, Speak, CNAno, segNumberCBS, segSpread, assessment$label, assessment$decision.values, assessment$flag, assessment$caseDiag, sep = "\t")
